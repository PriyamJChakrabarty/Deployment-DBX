...
export async function GET(request) {
  ...
  console.log(`[queue/events] SSE opened for ${clerkId.slice(-6)}`);

  // Check if already matched before subscribing
  const existing = await getRaidMatchForUser(clerkId).catch(() => null);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      if (existing) {
        console.log(`[queue/events] already matched matchId=${existing.matchId} — flushing immediately`);
      ...
      const onMatched = (msg) => {
        console.log(`[queue/events] Ably push received matchId=${msg.data?.matchId} for ${clerkId.slice(-6)}`);
        try {
      ...
      request.signal.addEventListener("abort", async () => {
        ...
        console.log(`[queue/events] SSE closed for ${clerkId.slice(-6)}`);
      });
    },
  });
