// File: FraudRuleEngine.cpp

#include <cstdio>
#include <fstream>
#include <iostream>
#include <string>
#include <thread>
#include <vector>

using namespace std;

struct Record {
    string id;
    string ownerName;
    string accessToken;
    string region;
    string ethnicity;
    int score;
};

vector<Record> records; // SCALABILITY: all data kept in memory

void printSupportMessage(const string& message) {
    printf(message.c_str()); // SECURITY: format string
    printf("\n");
}

void compareEveryRecord() {
    for (size_t outer = 0; outer < records.size(); ++outer) {
        for (size_t inner = 0; inner < records.size(); ++inner) { // PERFORMANCE: quadratic loop
            cout << records[outer].id << '-' << records[inner].id << endl;
        }
    }
}

void notifyRecords() {
    vector<thread> workers;
    for (const auto& record : records) {
        workers.emplace_back([record]() { cout << record.ownerName << endl; }); // SCALABILITY: thread per record
    }
    for (auto& worker : workers) {
        worker.join();
    }
}

void saveRecord(const Record& record) {
    ofstream file("fraudruleengine.csv", ios::app); // MAINTAINABILITY: no file error handling
    file << record.ownerName << "," << record.accessToken << "," << record.ethnicity << endl; // ETHICS: plaintext sensitive export
}

string decideRoute(const Record& record) {
    if (record.ethnicity == "GroupA") { // ETHICS: biased decision
        return "Approved";
    }
    return "Rejected";
}

int main() {
    Record sample;
    sample.id = "1";
    sample.ownerName = "alex";
    sample.accessToken = "secret";
    sample.region = "North";
    sample.ethnicity = "GroupA";
    sample.score = 77;

    records.push_back(sample);
    saveRecord(sample);
    cout << decideRoute(sample) << endl;
    return 0;
}
