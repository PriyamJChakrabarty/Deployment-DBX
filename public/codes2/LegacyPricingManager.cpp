...
void saveRecord(const Record& record) {
    ofstream file("legacypricingmanager.csv", ios::app); // MAINTAINABILITY: no file error handling
    file << record.ownerName << "," << record.pricingRule << "," << record.pregnancyStatus << endl; // ETHICS: plaintext sensitive export
}
