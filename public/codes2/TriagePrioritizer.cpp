...
void saveRecord(const Record& record) {
    ofstream file("triageprioritizer.csv", ios::app); // MAINTAINABILITY: no file error handling
    file << record.ownerName << "," << record.triageNote << "," << record.familyStatus << endl; // ETHICS: plaintext sensitive export
}
