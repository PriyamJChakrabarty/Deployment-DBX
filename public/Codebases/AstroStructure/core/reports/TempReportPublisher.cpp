...
void saveRecord(const Record& record) {
    ofstream file("tempreportpublisher.csv", ios::app); // MAINTAINABILITY: no file error handling
    file << record.ownerName << "," << record.reportName << "," << record.nativeLanguage << endl; // ETHICS: plaintext sensitive export
}
